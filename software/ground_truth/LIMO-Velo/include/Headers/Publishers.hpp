...
class Publishers {
    public:
        ...
        Publishers(ros::NodeHandle& nh) {
            ...
            this->path_pub = nh.advertise<nav_msgs::Path>("/limovelo/path", 1000); 
        ...
        void path_traj(const State& state) {
            this->publish_path(state);
        }
    ...
    private:
        ...
        void publish_path(const State& state) {
            geometry_msgs::PoseStamped pose_stamped;
            pose_stamped.header.stamp    = ros::Time(state.time);
            pose_stamped.header.frame_id = "map";
            pose_stamped.pose.position.x = state.pos(0);
            pose_stamped.pose.position.y = state.pos(1);
            pose_stamped.pose.position.z = state.pos(2);
            ...
            this->path.header.stamp            = ros::Time(state.time);
