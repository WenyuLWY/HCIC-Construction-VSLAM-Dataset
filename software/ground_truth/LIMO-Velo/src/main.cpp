...
int main(int argc, char** argv) {
    ...
    bool save_traj;
    nh.param("traj_path", traj_path, traj_path);
    nh.param("save_traj", save_traj, save_traj);
    ROS_WARN("%s",traj_path.c_str());
    ROS_WARN("%s",save_traj ? "true" : "false");
    if (save_traj)
        std::ofstream pose_file(traj_path, std::ios::out);
    ...
    while (ros::ok()) {
        ...
        while (accum.ready()) {
            ...
            // Step 1. LOCALIZATION
                ...
                publish.path_traj(Xt2);
                if (save_traj)
                {
                    std::ofstream pose_file(traj_path, std::ios::app);
                    pose_file.setf(std::ios::fixed, std::ios::floatfield);
                    if (pose_file.is_open())
                    {
                        Eigen::Quaternionf q(Xt2.R * Xt2.I_Rt_L().R);
                        pose_file << Xt2.time << " " << Xt2.pos(0) << " " << Xt2.pos(1) << " " << Xt2.pos(2) << " " 
